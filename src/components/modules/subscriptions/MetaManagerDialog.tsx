...
import type { ApiResponse, MetaOption } from "@/types";
...
type TabKey = "category" | "billing_cycle" | "status";
      ...
      <label className="relative h-6 w-6 cursor-pointer" title="Pick custom color">
        <div
          ...
          style={{
            ...
            border: isCustom ? "none" : "2px dashed var(--border)",
          }}
        />
        <input
          type="color"
...
export function MetaManagerDialog({ open, onClose }: Props) {
  const [activeTab, setActiveTab] = useState<TabKey>("category");
  const [meta, setMeta]           = useState<MetaGroup>({ categories: [], billingCycles: [], statuses: [] });
  ...
  // Edit form (replaces a list row inline)
  const [editTarget, setEditTarget]   = useState<MetaOption | null>(null);
  ...
  // Delete confirmation
  const [confirmDelete, setConfirmDelete] = useState<MetaOption | null>(null);
  ...
  // ── Tab switch — resets all inline forms ─────────────────────────────────────

  function switchTab(tab: TabKey) {
    setActiveTab(tab);
    setAddMode(false);   setAddLabel(""); setAddColor("#6366F1"); setAddError("");
    setEditTarget(null); setEditLabel(""); setEditColor("#6366F1"); setEditError("");
    setConfirmDelete(null); setDeleteError("");
  }

  function currentItems(): MetaOption[] {
    if (activeTab === "category")     return meta.categories;
    if (activeTab === "billing_cycle") return meta.billingCycles;
  ...
  async function handleAdd() {
    ...
    setAddError("");
    try {
      await api.post("/subscriptions/meta", { type: activeTab, label: addLabel.trim(), color: addColor });
      await fetchMeta();
      setAddMode(false);
      setAddLabel("");
      setAddColor("#6366F1");
  ...
  function startEdit(item: MetaOption) {
    setEditTarget(item);
  ...
  async function handleEdit() {
    ...
    setEditError("");
    try {
      await api.patch(`/subscriptions/meta/${editTarget.uuid}`, { label: editLabel.trim(), color: editColor });
      await fetchMeta();
      setEditTarget(null);
    } catch (err: unknown) {
      setEditError(
  ...
  function startDelete(item: MetaOption) {
  ...
  async function handleDelete() {
    ...
    setDeleteError("");
    try {
      await api.delete(`/subscriptions/meta/${confirmDelete.uuid}`);
      await fetchMeta();
      setConfirmDelete(null);
    } catch (err: unknown) {
      setDeleteError(
  ...
  function renderRow(item: MetaOption) {
    ...
    if (editTarget?.uuid === item.uuid) {
      return (
        <li key={item.uuid} className="space-y-3 rounded-lg p-3" style={{ background: "var(--bg-elevated)" }}>
          ...
          {editError && (
            <p className="text-xs rounded px-2 py-1" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444" }}>
              {editError}
            </p>
          )}
      ...
      return (
        <li key={item.uuid} className="rounded-lg p-3 space-y-2" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.2)" }}>
          <p className="text-sm" style={{ color: "var(--text-primary)" }}>
            Delete <strong>&ldquo;{item.label}&rdquo;</strong>?
          </p>
          {deleteError && (
            <p className="text-xs rounded px-2 py-1" style={{ background: "rgba(239,68,68,0.1)", color: "#EF4444" }}>
              {deleteError}
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Manage Meta Options</DialogTitle>
        </DialogHeader>
        ...
        {addMode ? (
          <div className="rounded-lg p-3 space-y-3" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border)" }}>
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={handleAdd} disabled={addSaving} className="h-7 text-xs px-3">
                {addSaving ? "Saving…" : "Add"}
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => { setAddMode(false); setAddLabel(""); setAddColor("#6366F1"); setAddError(""); }}
                className="h-7 text-xs px-3"
              >
                Cancel
              </Button>
          ...
          <button
            onClick={() => { setAddMode(true); setEditTarget(null); setConfirmDelete(null); }}
