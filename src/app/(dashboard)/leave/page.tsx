...
function BalanceCard({ label, used, total, color, index = 0 }: { label: string; used: number; total: number; color: string; index?: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground">{used} used of {total}</p>
    </div>
  );
}
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <div
              key={day}
              className={`rounded-lg border min-h-10 p-1 text-xs ${dow === 0 ? "bg-muted/30 border-border/50" : "border-border bg-card"}`}
            >
      ...
      <div className="space-y-6">
        ...
        {myBalance && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <BalanceCard index={3} label="Comp-Off"      used={0}                            total={Number(myBalance.compOff)}     color="text-primary" />
          </div>
        )}
              ...
              <tbody>
                {myRequests.length === 0 ? (
                  ...
                  <tr key={r.uuid} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                    <td className="px-4 py-3 font-medium text-foreground">{r.leaveType.replace("_"," ")}</td>
                    <td className="px-4 py-3 text-muted-foreground">{fmtDate(r.fromDate)}</td>
                    <td className="px-4 py-3 text-muted-foreground">{fmtDate(r.toDate)}</td>
        ...
        <div className="animate-fade-up rounded-2xl border border-border bg-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {pending.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-muted-foreground">No pending requests.</td></tr>
                ) : pending.map((r) => {
                  ...
                  return (
                    <tr key={r.uuid} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                      ...
                      <td className="px-4 py-3 text-foreground font-medium">{r.leaveType.replace("_"," ")}</td>
                      <td className="px-4 py-3 text-muted-foreground text-xs">{fmtDate(r.fromDate)} → {fmtDate(r.toDate)}</td>
                      <td className="px-4 py-3 text-center font-semibold text-foreground">{Number(r.days)}</td>
              ...
              <tbody>
                {allReqs.length === 0 ? (
                  ...
                  <tr key={r.uuid} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{r.leaveType.replace("_"," ")}</td>
                    <td className="px-4 py-3 text-muted-foreground">{fmtDate(r.fromDate)}</td>
                    <td className="px-4 py-3 text-muted-foreground">{fmtDate(r.toDate)}</td>
                    <td className="px-4 py-3 text-center font-semibold text-foreground">{Number(r.days)}</td>
                    <td className="px-4 py-3">
